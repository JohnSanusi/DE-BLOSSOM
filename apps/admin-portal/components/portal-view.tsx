...
export default function PortalView({ requiredRole }: { requiredRole: Role }) {
  ...
  const filteredTransactions = useMemo(
    () =>
      transactions.filter((row) =>
        `${row.description} ${row.account_type} ${row.transaction_type}`
          .toLowerCase()
  ...
  const filteredMembers = useMemo(
    () =>
      members.filter((member) =>
        `${member.full_name} ${member.email} ${member.member_number}`
          .toLowerCase()
          .includes(search.toLowerCase()),
      ),
    [members, search],
  ...
  const pageTitle: Record<View, string> = {
    dashboard: isAdmin ? "Admin overview" : "My dashboard",
    accounts: "Savings & accounts",
    transactions: "Transaction history",
    loans: "Loans & repayments",
    statements: "Statements",
    members: "Member directory",
    records: "Savings records",
  ...
  return (
    <div className="workspace">
      ...
      />
      <aside className={`workspace-nav ${mobileNav ? "open" : ""}`}>
        ...
        <div className="workspace-user">
          ...
          <div>
            ...
            <span>{isAdmin ? "Administrator" : profile.member_number}</span>
          </div>
        </div>
        <nav className="workspace-links">
          ...
          {isAdmin ? (
            ...
            <>
              <p>My finances</p>
              <button
                className={view === "accounts" ? "selected" : ""}
              ...
              <button
                className={view === "transactions" ? "selected" : ""}
              ...
              <button
                className={view === "statements" ? "selected" : ""}
      ...
      <main className="workspace-main">
        ...
        <div className="workspace-content">
          ...
          {view === "loans" && <Loans loans={loans} isAdmin={isAdmin} />}
          {view === "statements" && <Statements accounts={accounts} />}
          {view === "members" && (
            <Members
  ...
  return (
    <div className="page-stack">
      <div className="page-intro">
        <div>
          <p className="eyebrow">
            {isAdmin ? "Society at a glance" : "Your finances at a glance"}
          </p>
          <h2>
            {isAdmin
          ...
          <p className="muted">
            Here&apos;s what&apos;s happening across your cooperative workspace.
          </p>
        </div>
        <button
          className="primary-button"
          onClick={() => navigate(isAdmin ? "records" : "statements")}
        >
          <FileText size={17} />
          {isAdmin ? "Add savings record" : "Generate statement"}
        </button>
      </div>
        ...
        {isAdmin ? (
          <>
            ...
            <Metric
              ...
              helper={`${loans.filter((loan) => loan.status === "Active").length} active loans`}
              icon={<Landmark />}
              tone="slate"
            />
          </>
          ...
          <>
            ...
            <Metric
              ...
              value={money(outstanding)}
              helper={
                loans.length ? `${loans[0].status} account` : "No active loan"
              }
              icon={<Landmark />}
              tone="amber"
            />
      ...
      <div className="dashboard-columns">
          ...
          <SectionHead
            ...
            subtitle={
              isAdmin
                ...
                : "Your latest cooperative transactions"
            }
            action="View all"
            onClick={() => navigate(isAdmin ? "records" : "transactions")}
          />
          {transactions.length ? (
            <div className="activity-list">
              {transactions.slice(0, 5).map((row) => (
                <Activity key={row.id} row={row} />
              ))}
            </div>
          ) : (
            <Empty
              ...
              text={
                isAdmin
                  ? "New savings and repayment records will appear here."
                  : "Your transactions will appear here once records are added."
              }
            />
          )}
        </section>
        <section className="surface">
          <SectionHead
            title={isAdmin ? "Member directory" : "Account snapshot"}
            subtitle={
              isAdmin
                ? "Search and manage member accounts"
                : "Your membership details"
            }
            action={isAdmin ? "View members" : undefined}
            onClick={isAdmin ? () => navigate("members") : undefined}
          />
          {isAdmin ? (
            <div className="mini-members">
              {members.slice(0, 5).map((member) => (
                <div className="mini-member" key={member.id}>
              ...
              {!members.length && (
                <Empty
                  ...
                  text="New member profiles will show here."
                />
              )}
            </div>
          ) : (
            <div className="snapshot">
              <div>
                ...
                <strong>{profile.member_number}</strong>
              </div>
              <div>
                <span>Email</span>
              ...
              <div>
                <span>Accounts</span>
                <strong>{accounts.length} active</strong>
              </div>
              <div>
                <span>Loan status</span>
                <strong>{loans[0]?.status ?? "No loan"}</strong>
              </div>
            </div>
          )}
        </section>
...
function Activity({ row }: { row: Transaction }) {
  return (
    <div className="activity-row">
      <span
        className={`activity-icon ${row.transaction_type === "Credit" ? "credit" : "debit"}`}
      >
      ...
      <div>
        ...
        <span>
          {row.account_type} · {date(row.transaction_date)}
        </span>
      </div>
      <b
        className={row.transaction_type === "Credit" ? "positive" : "negative"}
      >
        {row.transaction_type === "Credit" ? "+" : "-"}
    ...
    <div className="page-stack">
      <div className="page-intro">
        <div>
          ...
          <h2>Savings, shares & special savings</h2>
          <p className="muted">
            Review balances and activity across each account type.
          </p>
        </div>
        <button
          className="primary-button"
      ...
      <div className="metric-grid three">
        {accounts.map((account) => (
          <Metric
            ...
            value={money(Number(account.balance))}
        ...
        {!accounts.length && (
          <Empty
            ...
            text="Your account balances will appear after the cooperative creates them."
          />
        )}
      </div>
      <section className="surface">
        ...
        {transactions.length ? (
          ...
          <Empty
            ...
            text="There is no activity for these accounts yet."
          />
        )}
      </section>
    </div>
    ...
    <div className="page-stack">
      <div className="page-intro">
        <div>
          ...
          <h2>Transactions</h2>
          <p className="muted">
            Search credits and debits across your cooperative records.
          </p>
        </div>
      </div>
      <section className="surface">
        ...
        />
        <DataTable
          headers={["Date", "Account", "Description", "Type", "Amount"]}
          rows={rows.map((row) => [
...
function Loans({ loans, isAdmin }: { loans: Loan[]; isAdmin: boolean }) {
  return (
    <div className="page-stack">
      <div className="page-intro">
        <div>
          <p className="eyebrow">
            {isAdmin ? "Administration" : "Borrowing & repayment"}
          </p>
          <h2>{isAdmin ? "Loan management" : "Loans & repayments"}</h2>
          <p className="muted">
            {isAdmin
              ? "Review active loans and repayment progress."
              : "Track your principal, interest, repayments, and outstanding balance."}
          </p>
        </div>
      </div>
      {loans.map((loan) => (
        <section className="loan-card surface" key={loan.id}>
          <div className="loan-summary">
            <div>
              <span className="eyebrow">{loan.status} loan</span>
              <h3>{money(Number(loan.outstanding_balance))}</h3>
          ...
          <div className="progress">
            <span
              style={{
                width: `${Math.min(100, (Number(loan.amount_paid) / Math.max(1, Number(loan.total_payable))) * 100)}%`,
              }}
            />
          </div>
          <div className="loan-stats">
            <div>
              ...
              <strong>{money(Number(loan.principal_amount))}</strong>
            </div>
            <div>
              <span>Total payable</span>
              <strong>{money(Number(loan.total_payable))}</strong>
            </div>
            <div>
              <span>Amount paid</span>
              <strong>{money(Number(loan.amount_paid))}</strong>
            </div>
            <div>
              <span>Due date</span>
              <strong>{date(loan.due_date)}</strong>
            </div>
          </div>
      ...
      {!loans.length && (
        <section className="surface">
          <Empty
            ...
            text={
              isAdmin
                ? "Active member loans will appear here."
                : "You do not have any loans recorded yet."
            }
          />
        </section>
      )}
...
function Statements({ accounts }: { accounts: Account[] }) {
  return (
    <div className="page-stack">
      <div className="page-intro">
        <div>
          ...
          <h2>Statements</h2>
          <p className="muted">
            Generate a statement for a selected account and date range.
          </p>
        </div>
      ...
      <section className="statement-builder surface">
        <div className="statement-form">
          ...
          <label>
            ...
            <select>
              ...
              {accounts.map((account) => (
                <option key={account.id}>{account.account_type}</option>
              ))}
            </select>
          </label>
        ...
        <div className="statement-preview">
          ...
          <h3>De-Blossom Cooperative Society</h3>
          <p>Member account statement</p>
    ...
    <div className="page-stack">
      <div className="page-intro">
        <div>
          ...
          <p className="muted">
            Search profiles by name, email, or member number.
          </p>
        </div>
    ...
    <div className="page-stack">
      <div className="page-intro">
        <div>
          ...
          <p className="muted">
            Add a record to a member&apos;s account using their member identity.
          </p>
        </div>
        <button className="secondary-button" onClick={() => navigate("loans")}>
          <Landmark size={16} />
          Manage loans
        </button>
      </div>
      <section className="surface record-form">
        <div className="section-head">
          <div>
            <h3>Add member record</h3>
            <p>The selected member will see this entry in their dashboard.</p>
          </div>
        </div>
        <form onSubmit={saveRecord}>
          ...
          {message && (
            <p
              className={
                message.startsWith("Record saved") ? "success-message" : "error"
              }
            >
              {message}
            </p>
      ...
      <section className="surface">
        <SectionHead
          ...
          subtitle="Latest entries from Supabase"
        />
        <DataTable
          headers={["Date", "Account", "Description", "Type", "Amount"]}
          rows={transactions
    ...
    <div className="page-stack">
      <div className="page-intro">
        <div>
          ...
          <h2>Reports & audit</h2>
          <p className="muted">
            A live operational snapshot for the cooperative.
          </p>
        </div>
      ...
      <div className="report-grid">
        ...
        <Metric
          ...
          helper="Member loan records"
          icon={<Landmark />}
          tone="slate"
        />
      </div>
      <section className="surface audit-note">
        <ShieldCheck size={20} />
        <div>
          <h3>Audit-ready workspace</h3>
          <p>
            Operational changes, imports, repayments, receipt access, and
            exports should be recorded here as audit events.
          </p>
        </div>
      </section>
    </div>
    ...
    <div className="page-stack">
      <div className="page-intro">
        <div>
          ...
          <p className="muted">Your authenticated cooperative identity.</p>
        </div>
      </div>
